...
MoveController::MoveController(std::string name):as_(nh_, name, boost::bind(&MoveController::executeCB, this, _1), false),action_name_(name),tf2_(tf2_buffer_)
{
    ...
    if(use_scan_)
    {
        ros::Subscriber sub1 = nh_.subscribe("/scan", 1, &MoveController::updateScan, this);
    }
    ros::Subscriber sub2 = nh_.subscribe("/odom", 5, &MoveController::updateOdom, this);
...
{   
    ...
    else if( current_goal_.distance<0 && std::fabs(current_goal_.angle)<=theta_torelance_)
    {
...
bool MoveController::getForwardError(float & e_theta,float & e_x)
{
    ...
    e_theta = dx*cos(theta) + dy*sin(theta);
    e_x = -dx*sin(theta) + dy*cos(theta);
    return true;
}
